...
class TrainTask(object):
    ...
    def build_model(self, use_weather=True):
        if self.model_name == "STGCNModel":
            ...
            self.index_mae = tf.reduce_mean(tf.abs(self.ouput_feature_flow_ph - self.output_student))
            self.index_rmse = tf.sqrt(self.loss_predict)
        else:
    ...
    def test(self):
        ...
        for step in range(self.eval_batch_num_per_epoch):
            ...
            feed_dict = {self.input_history_flow_ph: batch_flow_P,
                         ...
                         self.keep_prob: 1.0}
            loss, loss_all,mae,rmse = self.sess.run([self.loss_predict, self.loss,self.index_mae,self.index_rmse], feed_dict=feed_dict)
    ...
    def test_new(self):
        ...
        for step in range(self.eval_batch_num_per_epoch):
            ...
            feed_dict = {self.input_history_flow_ph: batch_flow_P,
                         ...
                         self.is_training_student_ph: False,
                         self.keep_prob: 1.0}
            output_predict = self.sess.run(self.output_student, feed_dict=feed_dict)
            flow_label.append(batch_flow_Q.copy())
            flow_predict.append(output_predict.copy())
            plan_label.append(batch_plan_Q.copy())
            weather_label.append(batch_weather[:, :, :, -cfg.train.output_Q:].copy())
        flow_label_all = np.concatenate(flow_label, axis=0)
        flow_predict_all = np.concatenate(flow_predict, axis=0)
        plan_label_all = np.concatenate(plan_label, axis=0)
        weather_label_all = np.concatenate(weather_label, axis=0)
        ...
        np.save(self.result_loss_dir + "/plan_label_test_result.npy", np.array(plan_label_all))
        np.save(self.result_loss_dir + "/weather_label_test_result.npy", np.array(weather_label_all))
