...
def st_conv_block(x, Ks, Kt, channels, scope, keep_prob, act_func='GLU', is_layer_normal=True):
    '''
...
def fully_con_layer(x, n, channel, channel_out, scope):
    '''
    ...
    :param scope: str, variable scope.
    :return: tensor, [batch_size, 1, n_route, channel_out].
    '''
    w = tf.get_variable(name=f'w_{scope}', shape=[1, 1, channel, channel_out], dtype=tf.float32)
    tf.add_to_collection(name='weight_decay', value=tf.nn.l2_loss(w))
    b = tf.get_variable(name=f'b_{scope}', initializer=tf.zeros([n, channel_out]), dtype=tf.float32)
    return tf.nn.conv2d(x, w, strides=[1, 1, 1, 1], padding='SAME') + b


def output_layer(x, Kt, time_step_out, scope, act_func='GLU'):
    '''
    ...
    with tf.variable_scope(f'{scope}_in'):
        x_i = temporal_conv_layer(x, Kt, channel, channel, act_func=act_func)
    x_ln = layer_norm(x_i, f'layer_norm_{scope}')

    _, time_step_in, _, _ = x_ln.get_shape().as_list()
    Kt_output = time_step_in - time_step_out + 1
    with tf.variable_scope(f'{scope}_out'):
        x_o = temporal_conv_layer(x_ln, Kt_output, channel, channel, act_func='sigmoid')
    # maps multi-channels to channel_out.
    x_fc = fully_con_layer(x_o, n, channel, 1, scope)
    return x_fc
